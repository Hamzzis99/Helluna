...
void UHeroGameplayAbility_InRepair::ActivateAbility(const FGameplayAbilitySpecHandle Handle, const FGameplayAbilityActorInfo* ActorInfo, const FGameplayAbilityActivationInfo ActivationInfo, const FGameplayEventData* TriggerEventData)
{
	...
	{
		AResourceUsingObject_SpaceShip* Ship = GetSpaceShip();
		const bool bShipFromGS = (Ship != nullptr);
		if (!Ship) { Ship = Cast<AResourceUsingObject_SpaceShip>(UGameplayStatics::GetActorOfClass(GetWorld(), AResourceUsingObject_SpaceShip::StaticClass())); }
		UE_LOG(LogTemp, Warning, TEXT("[Repair][BuildDiag] InRepair Ship=%s source=%s"), Ship ? *Ship->GetName() : TEXT("NULL"), bShipFromGS ? TEXT("GameState") : (Ship ? TEXT("FallbackGetActorOfClass") : TEXT("NONE")));
		if (Ship)
		{
