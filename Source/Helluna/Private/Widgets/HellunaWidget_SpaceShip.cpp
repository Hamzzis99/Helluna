...
void UHellunaWidget_SpaceShip::OnRepairChanged(int32 Current, int32 Need)
{
    ...
    if (TextBlock_0)
    {
        TextBlock_0->SetText(FText::FromString(FString::Printf(TEXT("%d / %d"), Current, Need)));
    }
}
...
void UHellunaWidget_SpaceShip::RefreshShipHP(float Health, float MaxHealth, float Percent)
{
    ...
    if (ShipStatusText)
    {
        const FString S = bDestroyed ? TEXT("파괴됨") : (bCritical ? TEXT("위험") : TEXT("정상"));
    ...
    if (bShipCritical != bCritical)
    {
        bShipCritical = bCritical;
        ShipPulseTime = 0.f;
        if (!bCritical && ShipHPBar)
        {
            ShipHPBar->SetRenderOpacity(1.f);
        }
    }
}

void UHellunaWidget_SpaceShip::NativeTick(const FGeometry& MyGeometry, float InDeltaTime)
{
    Super::NativeTick(MyGeometry, InDeltaTime);

    if (bShipCritical && ShipHPBar)
    {
        ShipPulseTime += InDeltaTime;
        const float Period = 0.8f;
        const float Phase = (ShipPulseTime / Period) * 2.f * PI;
        const float T = 0.5f + 0.5f * FMath::Sin(Phase);
        ShipHPBar->SetRenderOpacity(FMath::Lerp(0.3f, 1.0f, T));
    }
}
