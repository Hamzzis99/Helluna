...
// [Helluna fork] 외부 주입 widget 정의
TSharedPtr<SWidget> FAsyncLoadingScreenModule::ExternalLoadingWidget = nullptr;
...
void FAsyncLoadingScreenModule::SetupLoadingScreen(const FALoadingScreenSettings& LoadingScreenSettings)
{
	...
	// [Helluna fork] 외부 주입 widget이 있으면 우선 사용
	if (ExternalLoadingWidget.IsValid())
	{
		LoadingScreen.WidgetLoadingScreen = ExternalLoadingWidget.ToSharedRef();
	}
	else if (LoadingScreenSettings.bShowWidgetOverlay)
	{
		const ULoadingScreenSettings* Settings = GetDefault<ULoadingScreenSettings>();
